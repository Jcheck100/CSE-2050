class OrderedList:
    ...
    def __contains__(self, item):
        ...
        return self._bs(item, 0, len(self)-1) # You'll have to implement _bs() for this to work
    ...
    ################################################
    def _bs(self, val, left=0, right=None):
        if right is None:
            right = len(self._L) - 1  
        ...
        if self._L[mid] == val:
            return True
        elif self._L[mid] > val:
            return self._bs(val, left, mid-1)
        else:
            return self._bs(val, mid + 1, right)